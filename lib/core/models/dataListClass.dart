...
//
//     final dataListClass = dataListClassFromJson(jsonString);

import 'dart:convert';

DataListClass dataListClassFromJson(String str) =>
    DataListClass.fromJson(json.decode(str));

String dataListClassToJson(DataListClass data) => json.encode(data.toJson());
...
class DataListClass {
  ...
  List<TimeSeries> casesTimeSeries;
  List<Statewise> statewise;
  List<Tested> tested;

  factory DataListClass.fromJson(Map<String, dynamic> json) => DataListClass(
        casesTimeSeries: List<TimeSeries>.from(
            json["cases_time_series"].map((x) => TimeSeries.fromJson(x))),
